...
Race::Race(std::vector<Character*>& characters, Track& t)
    :RaceCharacterList(characters), track(t), playX(0), playY(7), cpuX(0), cpuY(8) {}
...
void Race::RaceStart()
{
    ...
    {
        // 플레이어 이동 처리
        playX += RaceCharacterList[0]->GetStatusSpeed() / 10;

        // CPU 이동 처리 (장애물이 없는 곳에서만 이동)
        cpuX += RaceCharacterList[1]->GetStatusSpeed() / 10;
